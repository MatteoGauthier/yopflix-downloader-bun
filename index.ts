...
async function getJson<T>(url: string): Promise<T> {
	const res = await fetch(url, {
		headers: {
			...
			"user-agent": "yopflix-downloader-bun/1.0 (Bun)"
		}
	});
	if (!res.ok) {
...
function isUqloadEmbed(url: string): boolean {
	try {
		const u = new URL(url);
		return (u.hostname.includes("uqload.") || u.hostname === "uqload.cx" || u.hostname === "uqload.net") && u.pathname.startsWith("/embed-");
	} catch {
		return false;
	}
}
...
async function cmdList(argsv: string[]) {
	...
	for (const v of videos) {
		const se = v.season && v.episode ? `S${String(v.season).padStart(2, "0")}E${String(v.episode).padStart(2, "0")}` : v.name;
